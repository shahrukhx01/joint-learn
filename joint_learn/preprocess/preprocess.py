...
class Preprocess:
    ...
    def perform_preprocessing(self, data, columns_mapping):
        ...
        data["clean_text"] = data[columns_mapping["text"]].apply(
            lambda text: text.lower()
        )
        ## removing usernames
        data["clean_text"] = data.clean_text.apply(self.remove_usernames)

        ## removing punctuations
        data["clean_text"] = data.clean_text.apply(self.remove_punctuations)

        ## removing stopwords
        data["clean_text"] = data.clean_text.apply(self.remove_stopwords)
